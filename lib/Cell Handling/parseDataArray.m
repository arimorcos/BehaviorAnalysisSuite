function dataCell = parseDataArray(dataCell,data)
...
tStarts = [1 find(diff(data(9,:)) == -1) + 1]; %find trial starts and add first column to account for first trial
tStops = find(diff(data(9,:)) == 1) + 1; %find trial stop
