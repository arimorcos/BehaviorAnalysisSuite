function moveArchivedToCurrent(mice,paths,folders)
%moveArchivedToCurrent.m Moves mice from archived mice folder to current
...
for i = 1:length(paths)
    ...
    for j = 1:length(mice)
        fileToMove = fullfile(archMice,mice{j});
        if exist(fileToMove,'dir')
            movefile(fileToMove,currMice,'f'); %move folder
        end
    end
end